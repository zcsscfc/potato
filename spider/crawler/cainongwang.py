...
def DownLoad(url_seed, url_pattern, origin_name):
	...
	while url_list:
		...
		try:
			print 'hi begin'
			fixedHtml = tree.cssselect('div.detail-box>div.detail-bd')[0]
			
			#imageTag = fixedHtml.cssselect('img')[0]
			imageTagSrc = 'http://tse1.mm.bing.net/th?&id=OIP.M47414a82f1769c862a1092405e06d80eo0&w=300&h=240&c=0&pid=1.9&rs=0&p=0'#imageTag.get('src')
			print imageTagSrc
			...
			titleText = tree.cssselect('div.detail-box>div.detail-hd>div.t-head>h2')[0].text.strip()
...
def HandleHtml2MySql(url, html, titleText, post_id, thumb):
	...
	ins_post_m.execute(post_id=post_id,title=titleText,digest='',thumb=thumb,\
		origin_id='7',create_t=create_t,from_url=url)
...
origin_name = '菜农网'
url_seed = 'http://www.cainong.cc/jishu/13309.html'
url_pattern = 'http://www.cainong.cc/jishu/'
