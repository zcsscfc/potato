...
def DownLoad(url_seed, url_pattern, origin_name):
	...
	while url_list:
		...
		try:
			print 'hi begin'
			fixedHtml = tree.cssselect('div#article')[0]
			
			#imageTag = fixedHtml.cssselect('img')[1]
			imageTagSrc = 'http://www.haogua.net/file/upload/201311/14/14-21-18-93-1.gif'#imageTag.get('src')
			print imageTagSrc
			...
			titleText = tree.cssselect('h1#title')[0].text.strip()
...
def HandleHtml2MySql(url, html, titleText, post_id, thumb):
	...
	ins_post_m.execute(post_id=post_id,title=titleText,digest='',thumb=thumb,\
		origin_id='13',create_t=create_t,from_url=url)
...
origin_name = '中国好瓜网'
url_seed = 'http://www.haogua.net/zhongzhi/100.html'
url_pattern = 'http://www.haogua.net/zhongzhi'
